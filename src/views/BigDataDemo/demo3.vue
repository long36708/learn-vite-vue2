...
export default {
  ...
  methods: {
    handleChangeItem(e, item) {
      console.log("handleChangeItem", e.target.checked, item);
      if (e.target.checked) {
        item.isChecked = true;
        this.checkedLabelKeys.push(item.id);
      } else {
        item.isChecked = false;
        // 注意变量不能重名!!!
        const index = this.checkedLabelKeys.findIndex((key) => key === item.id);
        if (index > -1) {
          this.checkedLabelKeys.splice(index, 1);
        }
      }
    },
    handleSetData() {
    ...
    handleCheckAll() {
      this.checkedLabelKeys = this.filteredLabelList.map((item) => {
        item.isChecked = true;
        return item.id;
      });
    ...
    handleCancelCheckAll() {
      ...
      this.filteredLabelList.map((item) => {
        item.isChecked = false;
      });
    },
  },
  computed: {
    visibleList() {
      ...
      const copyList = this.filteredLabelList.slice(start, end);
